...
import { Bell, Star, Plus, Menu } from 'lucide-react';
...
export default function Header() {
  ...
  return (
    <div className="w-full border-b border-gray-200 relative z-50">
      ...
      <header className="flex items-center justify-between px-4 py-3 bg-white">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="hidden sm:flex items-center border border-gray-300 rounded-md px-3 py-1 bg-white">
            <span className="text-black font-semibold text-sm">Market in uptrend</span>
            <div className="ml-2 w-6 h-6 rounded flex items-center justify-center">
              <img src="/market_uptrend.svg" alt="Market uptrend indicator" className="w-6 h-6" />
            </div>
          </div>

          {/* Icon buttons */}
          <div className="flex items-center space-x-1">
            <button type="button" className="p-2 rounded-full hover:bg-gray-100 transition-colors" aria-label="View favorites">
              <Star className="h-5 w-5 text-black" />
            </button>
            <button type="button" className="p-2 rounded-full hover:bg-gray-100 transition-colors" aria-label="View notifications">
              <Bell className="h-5 w-5 text-black" />
            </button>
            <button type="button" className="p-2 rounded-full hover:bg-gray-100 transition-colors" aria-label="Create new item">
              <Plus className="h-5 w-5 text-black" />
            </button>
            <button type="button" className="p-2 rounded-full hover:bg-gray-100 transition-colors" aria-label="Open menu">
          ...
          {/* Auth area */}
          <SignedOut>
            {/* single button → opens Clerk modal; pages still exist for deep links */}
          ...
          <SignedIn>
            {/* keep your avatar → opens your ProfileSidebar */}
            <button
              type="button"
              onClick={openSidebar}
      ...
      <div className="md:hidden px-4 py-2 border-b border-gray-200 bg-white">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-2">
            <div className="flex items-center">
              <span className="text-xs font-medium text-black mr-1">Uptrend</span>
              <img src="/market_uptrend.svg" alt="Market uptrend" className="w-4 h-4" />
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Your existing sidebar — now powered by Clerk data */}
      <SignedIn>
        <ProfileSidebar
